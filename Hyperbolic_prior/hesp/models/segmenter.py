...
def fpc_binary_proxy_loss(probabilities, labels, num_classes=11):
    """
    V2 修复版：绝对安全的二分类代理 Loss
    """
    # 1. 提取概率，并做第一道安全裁剪
    probs_clipped = tf.clip_by_value(probabilities, 1e-7, 1.0 - 1e-7)

    # 2. 累加异常概率
    raw_prob_anomaly = tf.reduce_sum(probs_clipped[..., 1:], axis=-1)

    # 3. 【核心修复：防爆安全锁】
    # 无论底层条件概率怎么加，强行把它死死按在 0 到 1 之间！绝不允许超过 1.0！
    prob_anomaly = tf.clip_by_value(raw_prob_anomaly, 1e-7, 1.0 - 1e-7)
    prob_bg = tf.clip_by_value(probs_clipped[..., 0], 1e-7, 1.0 - 1e-7)

    # 4. 真实标签二值化
    binary_labels = tf.cast(labels > 0, tf.float32)

    # 5. 计算 Binary Cross Entropy (此时绝不可能再出现负数)
    # 权重调整为温和的 1:10，不要用之前的 0:50 这种极端休克疗法了
    bce_loss = - (10.0 * binary_labels * tf.log(prob_anomaly) +
                  1.0 * (1.0 - binary_labels) * tf.log(prob_bg))
    bce_loss = tf.reduce_mean(bce_loss)

    # 6. 计算 Binary Dice Loss (依然只算异常区域)
    intersection = tf.reduce_sum(prob_anomaly * binary_labels, axis=0)
    union = tf.reduce_sum(prob_anomaly, axis=0) + tf.reduce_sum(binary_labels, axis=0)
    dice_score = (2.0 * intersection + 1e-5) / (union + 1e-5)
    dice_loss = 1.0 - tf.reduce_mean(dice_score)

    # 组合返回
    return 10.0 * dice_loss + bce_loss
...
class Segmenter(AbstractModel):
    ...
    def train_val_fn(self, features, labels, mode):
        ...
        # =================【新增：注册测谎节点】=================
        # 计算当前 batch 中类别 > 0 的像素总数 (即缺陷总数)
        defect_count = tf.reduce_sum(valid_labels)
        # 用 tf.identity 给它取个系统能认出来的名字
        tf.identity(defect_count, name="batch_defect_count")
        # ======================================================
        ...
        ## LOSS
        # 调用终极武器：Dice Loss
        cross_entropy = fpc_binary_proxy_loss(
            probabilities=valid_cprobs,
            labels=valid_labels,
