...
'''
CFG_DIR = os.path.abspath(__file__)
# 向上退三层目录：config -> hesp -> HyperbolicImageSegmentation-main
...
class FPCConfig(DatasetConfig):
    # ================= 数据集基础配置 =================
    _NAME = 'fpc'
    # 指向您存放原始图片的根目录 (对应原框架的 dataset 目录)
    _DATASET_DIR = r'/home/cgz/lzc/HyperbolicImageSegmentation-main/datasets/fpc/datasets'
    # 指向存放 TFRecord 和 Json/Txt 的目录 (对应原框架的 data 目录)
    _DATA_DIR = r'/home/cgz/lzc/HyperbolicImageSegmentation-main/datasets/fpc/data'

    # 框架在训练中每个 Epoch 会读取它进行 mIoU 评估
    _TEST_FILE = os.path.join(_DATA_DIR, 'fpc_val.record')

    # 类别数：10 类缺陷 + 1 类背景 = 11
    _NUM_CLASSES = 11

    # ================= 训练超参数配置 =================
    # ⚠️ 请填入打包 Train 时控制台输出的真实切片数 (比如 12000)
    _NUM_TRAIN = 3531
    # ⚠️ 请填入打包 Val 时控制台输出的真实切片数 (我们稍后去打包一下)
    _NUM_VALIDATION = 668
    _NUM_EPOCHS = 60
    _INITIAL_LEARNING_RATE = 1e-3
    ...
    # ================= 双曲层级分类必备文件 =================
    _JSON_FILE = os.path.join(_DATA_DIR, 'FPC_hierarchy.json')
    _I2C_FILE = os.path.join(_DATA_DIR, 'FPC_i2c.txt')

    # 直接启用 txt2dict 函数读取类别名称映射
    _I2C = txt2dict(_I2C_FILE)
