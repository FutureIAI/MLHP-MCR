...
# colour map
label_colours_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'label_colours.npy')
...
def parse_record(raw_record):
    ...
    keys_to_features = {
        ...
        # 👇 核心修改 1：把 "label/encoded" 改成我们打包时的名字 👇
    ...
    # 👇 核心修改 2：去读我们真正的掩码名字 👇
    label = tf.image.decode_image(tf.reshape(parsed["image/segmentation/class/encoded"], shape=[]), 1)
    label = tf.to_int32(tf.image.convert_image_dtype(label, dtype=tf.uint8))
    label.set_shape([None, None, 1])
